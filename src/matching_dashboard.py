...
class MatchingDashboardError(ValueError):
    #report invalid matching input/output
    pass
...
def load_patient_matches(path: str | Path, patient_id: str) -> pd.DataFrame:
    #load selected patient's rows from the combined res csv
...
def load_patient_features(path: str | Path, patient_id: str) -> pd.DataFrame:
    #load 1 patient’s aligned lesion locations in FU voxel grid
...
def resolve_match_image_focus(
    ...
    #resolve a matching row to its BL and FU voxel locations
    missing = sorted(REQUIRED_FEATURE_COLUMNS - set(features.columns))
    if missing:
        raise MatchingDashboardError(
    ...
    def location(timepoint: str, column: str) -> LesionLocation | None:
        ...
        return LesionLocation(
            lesion_id, timepoint, centroid,
            int(row["component_index"]) if pd.notna(row.get("component_index")) else None,
            int(row["connectivity"]) if pd.notna(row.get("connectivity")) else 18,
            str(row["mask_path"]) if pd.notna(row.get("mask_path")) else None,
            str(row.get("mask_role", "")),
        )
...
def save_patient_matches(
    ...
    #replace 1 patient's results and keep all other patients' rows
...
def run_patient_match(
    ...
    #run min-cost-flow matching for the selected patient and save res
    selected_id = str(patient_id).strip()
